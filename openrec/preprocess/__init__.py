...
class RotateTall:
    """Rotate tall images (h > ratio*w) by 90° in the configured direction.

    `direction`: "cw" or "ccw". UnionST paper convention is `direction='ccw'`
    (90° counterclockwise) for images with height > 1.5 * width.
    Works on PIL Image or HWC ndarray.
    """

    def __init__(self, ratio=1.5, direction='cw', **kwargs):
        self.ratio = ratio
        self.direction = direction.lower()
        assert self.direction in ('cw', 'ccw'), f"direction must be 'cw' or 'ccw', got {direction}"
    ...
    def __call__(self, data):
        ...
        if isinstance(img, Image.Image):
            ...
            if h > self.ratio * w:
                img = img.transpose(
                    Image.ROTATE_90 if self.direction == 'ccw' else Image.ROTATE_270)
        else:
            arr = np.asarray(img)
            h, w = arr.shape[0], arr.shape[1]
            if h > self.ratio * w:
                img = np.rot90(arr, k=1 if self.direction == 'ccw' else -1).copy()
        data['image'] = img
        return data
