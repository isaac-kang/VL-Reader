...
class PARSeqAugPILPP(object):
    """PARSeqAugPIL + UnionST 'PP' (paper §9) add-ons for synthetic-data training.

    Pipeline order on each PIL image:
      1. Base RandAugment (same as PARSeqAugPIL).
      2. Online resampling: downsample to scale ∈ U[resample_min, resample_max] of
         original size, then upsample back. Always applied; 1.0× is a no-op so
         the random scale itself acts as the probability gate.
      3. JPEG re-encode at random quality ∈ U[jpeg_q_lo, jpeg_q_hi], with
         probability `compress_p` (paper §9: 0.2).
    """
    ...
    def __init__(self,
                 ...
                 **kwargs):
        from .parseq_aug import rand_augment_transform
        self.base = rand_augment_transform()
        self.compress_p = compress_p
        self.resample_min = resample_min
        self.resample_max = resample_max
        self.jpeg_q = tuple(jpeg_q)

    def _resample(self, img):
        s = random.uniform(self.resample_min, self.resample_max)
        if s >= 0.999:
            return img
        w, h = img.size
    ...
    def _compress(self, img):
        ...
        mode = img.mode
        save_img = img if mode in ('RGB', 'L') else img.convert('RGB')
        buf = io.BytesIO()
        save_img.save(buf, format='JPEG', quality=q)
        buf.seek(0)
        return Image.open(buf).convert(mode)

    def __call__(self, data):
        img = self.base(data['image'])
        img = self._resample(img)
        if random.random() < self.compress_p:
