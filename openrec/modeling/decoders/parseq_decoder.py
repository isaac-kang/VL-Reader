...
class _RMSNorm(nn.Module):
    # Fallback for torch versions without nn.RMSNorm.
...
def _make_rmsnorm(dim: int) -> nn.Module:
    return nn.RMSNorm(dim) if hasattr(nn, 'RMSNorm') else _RMSNorm(dim)


class DifferentialMultiheadAttention(nn.Module):
    """Differential Cross-Attention (Ye et al., 2024 / CSD §4.3, Eq. 11).

    Each head's q/k are split in half along the head dim; two softmax-attention
    maps are computed and the second is subtracted with a per-layer-shared
    scalar λ that is reparametrized as
        λ = exp(λ_q1·λ_k1) − exp(λ_q2·λ_k2) + λ_init
    Per-head outputs are RMSNorm'd (sub-LN) and scaled by (1 − λ_init) before
    the output projection. API mirrors nn.MultiheadAttention(batch_first=True)
    so DecoderLayer can swap implementations without other changes.
    """
    ...
    def __init__(self, d_model: int, num_heads: int, layer_idx: int = 0,
        ...
        assert self.head_dim % 2 == 0, 'head_dim must be even for diff split'
        self.half_dim = self.head_dim // 2
        self.scale = self.half_dim ** -0.5
        ...
        self.lambda_q1 = nn.Parameter(torch.empty(self.half_dim).normal_(0, 0.1))
        self.lambda_k1 = nn.Parameter(torch.empty(self.half_dim).normal_(0, 0.1))
        self.lambda_q2 = nn.Parameter(torch.empty(self.half_dim).normal_(0, 0.1))
        self.lambda_k2 = nn.Parameter(torch.empty(self.half_dim).normal_(0, 0.1))

        self.head_norm = _make_rmsnorm(self.head_dim)
    ...
    def forward(self, query: Tensor, key: Tensor, value: Tensor,
        ...
        B, Lq, D = query.shape
        Lk = key.shape[1]
        H, Hd, Hh = self.num_heads, self.head_dim, self.half_dim
        ...
        if key_padding_mask is not None:
            ...
            a1 = a1.masked_fill(kpm, float('-inf'))
            a2 = a2.masked_fill(kpm, float('-inf'))

        p1 = self.attn_dropout(F.softmax(a1, dim=-1))
        p2 = self.attn_dropout(F.softmax(a2, dim=-1))
...
class SwiGLU(nn.Module):
    """SwiGLU MLP (CSD §4.3): out = W3(SiLU(xW1) ⊙ xW2). Hidden dim is kept
    at the configured dim_feedforward (no 2/3 shrinking) to match Table 1's
    PLD-Diff parameter count (24.4M)."""
    ...
    def forward(self, x: Tensor) -> Tensor:
        return self.w_out(self.dropout(F.silu(self.w_gate(x)) * self.w_in(x)))
...
class DecoderLayer(nn.Module):
    ...
    def __init__(
        ...
        self.swiglu = swiglu
        if differential:
            self.self_attn = DifferentialMultiheadAttention(
                d_model, nhead, layer_idx=layer_idx, dropout=dropout)
            self.cross_attn = DifferentialMultiheadAttention(
                d_model, nhead, layer_idx=layer_idx, dropout=dropout)
        else:
            ...
            self.cross_attn = nn.MultiheadAttention(d_model,
                                                    ...
                                                    batch_first=True)
        if swiglu:
            self.ff = SwiGLU(d_model, dim_feedforward, dropout=dropout)
        else:
...
class Decoder(nn.Module):
    ...
    def __init__(self, decoder_layer, num_layers, norm, layers=None):
        super().__init__()
...
class PARSeqDecoder(nn.Module):
    ...
    def __init__(self,
        ...
        # Project encoder features to decoder embed_dim when they differ.
        # Lets a leaner backbone (e.g. SVTRv2 d=384) feed an unmodified
        # PLD-Base / PLD-Diff (d=768) without scaling the encoder up.
        self.memory_proj = (nn.Linear(in_channels, embed_dim)
                            if in_channels != embed_dim else nn.Identity())

        if differential or swiglu:
            # PLD-Diff path: each layer needs its own λ_init seeded by layer
            # index, so build layers explicitly instead of cloning.
            decoder_layers = [
                DecoderLayer(embed_dim, dec_num_heads,
                             embed_dim * dec_mlp_ratio, dropout,
                             differential=differential, swiglu=swiglu,
                             layer_idx=i + 1)
                for i in range(dec_depth)
            ]
            self.decoder = Decoder(None, dec_depth,
